...
const projectsData = [
  { title: "GLAM-ID", type: "Capstone Project", desc: "An NFC-based loyalty and client information system for FG Aesthetic Centre. Built with Python Flask and MySQL.", tags: ["Python", "Flask", "NFC", "MySQL"] },
  { title: "Product & UI/UX Design", type: "Capstone Project", desc: "Led the visual identity and product design using Adobe Photoshop, including mock-up booth designs and the overall website interface.", tags: ["Photoshop", "UI/UX", "Product Design", "Web Design"] },
  { title: "Portfolio Website", type: "Web Development", desc: "A personal portfolio website built with React, Tailwind CSS, and Framer Motion for smooth animations and elegant UI.", tags: ["React", "Tailwind CSS", "Framer Motion", "Vite", "HTML5", "CSS3", "JavaScript"] },
  { title: "Digital Illustration & Branding", type: "Creative Design", desc: "Created character concept art and brand identity frameworks using Medibang Paint Pro, Canva, and Adobe Illustrator.", tags: ["Illustration", "Branding", "Canva"] },
  { title: "GIG Market Designing", type: "NTEK Systems Inc.", desc: "Conducted application testing, market research, and SWOT analysis for the GIG Market app, alongside designing social media marketing materials.", tags: ["Marketing", "Social Media Design"] },
  { title: "Blender 3D Modelling", type: "Personal Project", desc: "Designed and rendered a low-poly 3D scene in Blender, showcasing skills in modeling, texturing, and lighting for conceptual art.", tags: ["3D Modeling", "Texturing", "Lighting"] }
];
...
const Projects = () => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      } catch (error) { console.log("Database offline."); }
    };
    fetchProjects();
  }, []);

  useEffect(() => {
    const timer = setInterval(() => setCurrentIndex((prev) => (prev + 1) % liveDesigns.length), 3500);
    return () => clearInterval(timer);
  }, [liveDesigns.length]);

  useEffect(() => {
    const autoTimer = setInterval(() => setAutoIndex((prev) => (prev + 1) % automationSlides.length), 10000);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="relative w-full flex flex-col lg:flex-row items-center justify-between gap-12 mb-32">
          <div className="w-full lg:w-5/12 z-20 order-2 lg:order-1 text-center lg:text-left">
             <h2 className="text-xs sm:text-sm font-bold text-rose-800 tracking-[0.2em] uppercase mb-6 flex items-center justify-center lg:justify-start gap-2"><Sparkles size={16} /> Featured Designs Gallery</h2>
             <div className="h-[200px] sm:h-[180px] relative"> 
                <AnimatePresence mode="wait">
                   <motion.div key={currentIndex} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }} className="absolute inset-0">
                     <h3 className="text-3xl sm:text-4xl md:text-5xl font-black text-black dark:text-white mb-4 leading-tight tracking-tighter">{liveDesigns[currentIndex]?.title}</h3>
                     <div className="text-gray-600 dark:text-gray-400 text-sm md:text-base leading-relaxed mb-6 font-light">{liveDesigns[currentIndex]?.expl}</div>
                     <button onClick={() => setIsAlbumOpen(true)} className="flex items-center gap-2 bg-rose-800 hover:bg-rose-900 text-white px-6 py-3 rounded-xl font-bold transition-all mx-auto lg:mx-0 shadow-lg hover:scale-105"><Images size={18} /> View Design Album</button>
                   </motion.div>
                </AnimatePresence>
             </div>
          </div>
          <div className="w-full lg:w-7/12 h-[300px] sm:h-[400px] md:h-[450px] relative flex justify-center items-center order-1 lg:order-2">
             {liveDesigns.map((proj, index) => {
                ...
                let animationProps = diff === 0 ? { x: 0, y: 0, scale: 1, opacity: 1, zIndex: 30, rotate: 0 } : diff === 1 ? { x: "12%", y: "-6%", scale: 0.9, opacity: 0.7, zIndex: 20, rotate: 3 } : diff === 2 ? { x: "24%", y: "-12%", scale: 0.8, opacity: 0.3, zIndex: 10, rotate: -2 } : { x: "30%", y: "-15%", scale: 0.7, opacity: 0, zIndex: 0 };
                return (
                  <motion.div key={proj.id} animate={animationProps} transition={{ duration: 0.8, ease: [0.32, 0.72, 0, 1] }} className="absolute w-4/5 md:w-[85%] aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl border bg-black cursor-pointer" onClick={() => diff === 0 && setSelectedDesign({ ...proj, fullImg: proj.fullImg, expl: proj.expl, tag: 'Design Sample' })}>
                     {proj.video && diff === 0 ? <video src={proj.video} className="w-full h-full object-cover pointer-events-none" autoPlay loop muted playsInline /> : <img src={proj.img || proj.fullImg} alt={proj.title} className="w-full h-full object-cover" />}
        ...
        <div className="border-t border-gray-200 dark:border-gray-800 pt-16 mt-16">
            ...
            {projectsData.map((item, index) => <ProjectCard key={index} {...item} />)}
            </div>
        </div>
      </motion.div>

      {/* --- CINEMATIC POP-OUT MODAL (USED FOR BOTH DESIGN & AUTOMATION) --- */}
      <AnimatePresence>
        {selectedDesign && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedDesign(null)} className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md p-4 flex items-center justify-center cursor-pointer">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-neutral-900 rounded-3xl max-w-6xl w-full max-h-[95vh] flex flex-col overflow-hidden border border-gray-800 shadow-2xl relative cursor-default">
                <button onClick={() => setSelectedDesign(null)} className="absolute top-6 right-6 p-2 rounded-full bg-black/50 text-white z-50"><X size={20}/></button>
                <div className="w-full bg-black flex items-center justify-center p-8 min-h-[40vh] border-b border-gray-800">
                    {selectedDesign.gallery ? (
                        <div className="relative w-full flex items-center justify-center">
                            <img src={selectedDesign.gallery[modalGalleryIndex]} className="max-h-[60vh] object-contain" />
                            {selectedDesign.gallery.length > 1 && (
                                <>
                                    <button onClick={() => setModalGalleryIndex(p => p === 0 ? selectedDesign.gallery.length - 1 : p - 1)} className="absolute left-4 p-3 bg-black/50 text-white rounded-full"><ChevronLeft /></button>
                                    <button onClick={() => setModalGalleryIndex(p => p === selectedDesign.gallery.length - 1 ? 0 : p + 1)} className="absolute right-4 p-3 bg-black/50 text-white rounded-full"><ChevronRight /></button>
                                </>
                            )}
                        </div>
                    ) : (
                        <img src={selectedDesign.fullImg} className="max-h-[60vh] object-contain" />
                    )}
                </div>
                <div className="p-8 overflow-y-auto">
                    <h3 className="text-4xl font-black mb-4 uppercase">{selectedDesign.title}</h3>
                    <p className="text-lg text-gray-400 mb-6">{selectedDesign.expl}</p>
                    {selectedDesign.downloadFile && (
                        <a href={selectedDesign.downloadFile} download className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-rose-800 text-white font-bold"><Download size={20} /> Download Sample File</a>
                    )}
                </div>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
            {isAlbumOpen && (
                <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed inset-0 z-[100] bg-white/95 dark:bg-neutral-950/95 backdrop-blur-xl overflow-y-auto p-6 md:p-12">
                    <button onClick={() => setIsAlbumOpen(false)} className="absolute top-6 right-6 p-3 bg-neutral-800 text-white rounded-full"><X size={24} /></button>
                    <div className="max-w-6xl mx-auto mt-12 pb-20">
                        <h2 className="text-5xl font-black mb-12">Design Album</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {albumMedia.map((media) => (
                                <div key={media.id} onClick={() => setSelectedAlbumMedia(media)} className="aspect-square rounded-2xl overflow-hidden cursor-pointer bg-neutral-900">
...
const ProjectCard = ({ title, type, desc, tags }) => (
  <motion.div whileHover={{ y: -5 }} className="flex flex-col h-full p-8 rounded-3xl bg-white dark:bg-neutral-900 border border-gray-800 cursor-default">
    <span className="text-xs font-bold text-rose-500 uppercase mb-2">{type}</span>
    <h3 className="text-2xl font-bold mb-4">{title}</h3>
    <p className="text-base text-gray-400 mb-6 flex-grow">{desc}</p>
    <div className="flex flex-wrap gap-2">{tags.map(t => <span key={t} className="px-3 py-1 bg-neutral-800 text-xs rounded-full">{t}</span>)}</div>
  </motion.div>
);
